...
<head>
    ...
    <title>{{ $profile->nama ?? 'Villa Serenity' }}</title>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-villa">
        <div class="container">
            <a class="navbar-brand-villa" href="/"><i class="bi bi-houses-fill me-1"></i> {{ $profile->nama ?? 'Villa Serenity' }}</a>
    ...
    <section class="hero-section text-center" style="padding: 5rem 0 7rem;">
        <div class="container" data-aos="fade-up">
            <h1 class="hero-title display-4">{{ $profile->nama ?? 'Villa Serenity' }}</h1>
            <p class="hero-subtitle lead">{{ $profile->deskripsi ?? 'Pengalaman menginap mewah dengan pemandangan alam yang menenangkan.' }}</p>
            @if($profile)
            <div class="d-flex justify-content-center gap-4 mt-4 text-white flex-wrap">
                <span><i class="bi bi-people-fill me-1"></i> {{ $profile->kapasitas }} orang</span>
                <span><i class="bi bi-door-closed-fill me-1"></i> {{ $profile->jumlah_kamar_tidur }} kamar tidur</span>
                <span><i class="bi bi-tag-fill me-1"></i> Rp {{ number_format($profile->harga_per_malam, 0, ',', '.') }}/malam</span>
            </div>
            @endif
    ...
    <div class="container" style="margin-top:-60px; position:relative; z-index:10;">
        <div class="detail-card" data-aos="zoom-in">
            <img src="{{ Storage::url($profile->foto_utama) }}" class="w-100" style="max-height:480px; object-fit:cover;">
        </div>
    </div>
    @endif
    ...
    @if($profile && $profile->fasilitas_umum)
    <div class="container py-4" id="fasilitas">
        <div class="glass-panel p-4 text-center" data-aos="fade-up">
            ...
            <div class="d-flex justify-content-center flex-wrap gap-2">
                @foreach(explode(', ', $profile->fasilitas_umum) as $f)
    ...
    <div class="container py-5 text-center" data-aos="fade-up">
        <div class="form-card" style="max-width:600px; margin:0 auto;">
            ...
            <p class="text-muted">Booking sekarang dan nikmati pengalaman menginap terbaik di {{ $profile->nama ?? 'villa kami' }}.</p>
    ...
    <footer class="footer-villa">
        <div class="container">
            <p class="mb-1">{{ $profile->alamat ?? '' }}</p>
            <p class="mb-1">{{ $profile->no_telp ?? '' }}</p>
            <p class="mb-0">{{ $profile->nama ?? 'Villa Serenity' }} &copy; {{ date('Y') }}</p>
        </div>
    </footer>
