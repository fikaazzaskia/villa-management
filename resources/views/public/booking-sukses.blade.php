...
<body class="d-flex align-items-center justify-content-center min-vh-100" style="background: linear-gradient(135deg, var(--wild-berry), var(--soft-rose));">
    <div class="auth-card text-center">
        ...
        <div class="text-start mt-4 p-3" style="background: var(--powder-grey); border-radius: 12px;">
            <p class="mb-1"><strong>Nama:</strong> {{ $booking->nama_tamu }}</p>
            <p class="mb-1"><strong>Check-in:</strong> {{ \Carbon\Carbon::parse($booking->tanggal_checkin)->format('d M Y') }}</p>
            <p class="mb-1"><strong>Check-out:</strong> {{ \Carbon\Carbon::parse($booking->tanggal_checkout)->format('d M Y') }}</p>
